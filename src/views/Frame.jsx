...
const Frame = () => {
  ...
  const fetchFrameProducts = async () => {
    setLoading(true);
    try {
      const products = await getProductsByType("frame");
      setFrameItems(products);
    } catch (error) {
  ...
  const handleSaveFrame = async () => {
    ...
    try {
      ...
      await addProduct(newFrameItem);
      const updatedProducts = await getProductsByType("frame");
      setFrameItems(updatedProducts);
      ...
      Alert.alert(
        "Error",
        "There was an error adding the frame product. Please try again."
      );
    } finally {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity className="bg-white rounded-lg shadow-lg p-4 mb-4 border">
      ...
      <TouchableOpacity
        onPress={() => handleDeleteFrame(item._id)}
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalContainer}>
              <Text className="text-2xl font-bold mb-4">Add Frame</Text>
              <TouchableOpacity
                onPress={pickImage}
                className="mb-4"
                disabled={isUploading}
